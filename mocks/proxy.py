...
def main():
    with console.status("[bold green]Generating proxy servers...") as status:
        ...
        for i in range(50):
            ...
            Proxy.objects.create(ip=ip, port=port, username=username, password=password, proxy_type=proxy_type, server_id=server_id, acl_ids=acl_ids, order_id=order_id, uid=uid, expired_at=expired_at)
